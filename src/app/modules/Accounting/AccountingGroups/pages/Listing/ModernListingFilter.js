import { Button, Collapse, IconButton, MenuItem } from "@material-ui/core";
import {
  Add,
  Clear,
  ExpandLess,
...
const ModernListingFilter = (props) => {
  ...
  const getReportFormatted = (report) => {
    ...
    const newReport = report.map((e) => {
      const idEntity =
        entities.find((entity) => entity.id == e.entity)?.entity || e.entity;
      const idCurrency =
        currency.find((c) => c.id == e.currency)?.currency || e.currency;
      return {
        ...e,
        idEntity,
        idCurrency,
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="d-flex justify-content-between align-items-center mb-4">
                ...
                <div className="d-flex align-items-center">
                  ...
                  <Button
                    ...
                    size="large"
                    onClick={() =>
                      history.push(`/accounting/accounting-groups/new`)
                    }
                    style={{
                      borderRadius: "8px",
                      padding: "12px 24px",
              ...
              <div className="d-flex justify-content-between align-items-center mt-4 pt-4 border-top">
                <div className="text-muted font-size-sm">
                  {props.data.length > 0 ? (
                    <span>
                      Mostrando <strong>{props.data.length}</strong> grupos
                      contables
                    </span>
                  ) : (
                    <span>No se encontraron grupos contables</span>
                  )}
                </div>
                <DownloadArchive
