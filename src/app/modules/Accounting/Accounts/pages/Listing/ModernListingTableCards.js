...
export default function ModernListingTableCards({ accountsData }) {
  return (
    <div className="row">
      {accountsData.length > 0 ? (
        accountsData.map((item) => {
          const Icon = getAccountIcon(item.account);
          const colors = getAccountColor(item.account);
          ...
          >
            <CardBody className="p-8 text-center">
              ...
              <p className="text-muted mb-0">
                No hay cuentas contables disponibles para mostrar
              </p>
            </CardBody>
          </Card>
        </div>
