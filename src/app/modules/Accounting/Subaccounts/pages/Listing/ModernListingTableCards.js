...
const getSubaccountIcon = (subaccountName) => {
  const name = subaccountName.toLowerCase();
...
const getSubaccountColor = (subaccountName) => {
  const name = subaccountName.toLowerCase();
...
export default function ModernListingTableCards({ subaccountsData }) {
  return (
    <div className="row">
      {subaccountsData.length > 0 ? (
        subaccountsData.map((item) => {
          const Icon = getSubaccountIcon(item.subAccount);
          const colors = getSubaccountColor(item.subAccount);
          ...
          >
            <CardBody className="p-8 text-center">
              ...
              <p className="text-muted mb-0">
                No hay subcuentas contables disponibles para mostrar
              </p>
            </CardBody>
          </Card>
        </div>
