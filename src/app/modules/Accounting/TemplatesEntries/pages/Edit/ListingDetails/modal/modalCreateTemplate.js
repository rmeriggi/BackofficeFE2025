...
import { createDetail } from "../../../../utils/service";
...
export function ModalCreateTemplate(props) {
    ...
    const [subAccountId, setSubAccountId] = useState(accounts[0]?.id)
    const [auxAccounts, setAuxAccounts] = useState(props.auxiliariesAccounts)
    const [auxAccountId, setAuxAccountId] = useState(accounts[0]?.id)
    const { open, variant,message, handleClose, setOpenMessage } = useSnackBar();

    const initialValues = {
        idHeader: props.idHeader,
        idAuxiliary: auxAccountId || 1,
    ...
    const editTemplate = async (values) => {
        ...
        const bodyDetail= {
            idHeader: values.idHeader,
            idAuxiliary: Number(auxAccountId),
            movementType: values.movementType
        }

        try {
            await createDetail(bodyDetail)
    ...
    const getAccountsByGroup = (idGroup) => {
        const accountsFiltered = props.accounts.filter(a => Number(a.group) === Number(idGroup))
        return accountsFiltered
    }
    
    const getSubAccountsByAccount = (idAccount) => {
        const subaccounts = props.subAccounts.filter(sa => Number(sa.account) === Number(idAccount))
        return subaccounts
    }

    const getAuxAccountsBySubAccount = (idSubAccount) => {
        const auxaccounts = props.auxiliariesAccounts.filter(aux => Number(aux.idSubAccount) === Number(idSubAccount))
        return auxaccounts
    }
