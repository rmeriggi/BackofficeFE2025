...
import { getSectionB } from "../../utils/service";
...
export default function Listing() {
  const [sectionBList, setSectionBList] = useState({ sectionB: [] });
  ...
  useEffect(() => {
    const cargarMesActual = async () => {
      ...
      try {
        ...
        const toYear = getYear(today);
        const toMonth = getMonth(today) + 1;
        ...
        const response = await getSectionB(dataValues);
        setSectionBList(response);
        setFechaDesde(today);
        setFechaHasta(today);
      } catch (error) {
  ...
  const processedData = useMemo(() => {
    if (!sectionBList?.sectionB) return [];
    ...
    return toReport.map((item, index) => ({
      ...
      esquemaDePago: item.paymentScheme || "",
  ...
  const registrosFiltrados = processedData.filter((r) => {
    ...
    const coincideBusqueda =
      ...
      r.medioDePago.toLowerCase().includes(busqueda.toLowerCase()) ||
      r.esquemaDePago.toLowerCase().includes(busqueda.toLowerCase()) ||
  ...
  const montoTotal = processedData.reduce((sum, r) => sum + (r.monto || 0), 0);
  ...
  const getTipoIcono = (tipo) => {
    switch (tipo.toLowerCase()) {
      case "pagos":
        return <Payment style={{ fontSize: 30, color: "#3699FF" }} />;
      case "esquemas":
        return <AccountBalance style={{ fontSize: 30, color: "#0BB783" }} />;
      case "medios":
        return <CreditCard style={{ fontSize: 30, color: "#FFA800" }} />;
      case "cantidad":
  ...
  /*   const getCategoriaIcono = (categoria) => {
    switch (categoria.toLowerCase()) {
      ...
      case "esquema":
        return <AccountBalance style={{ fontSize: 18, color: "#3699FF" }} />;
      case "medio":
        return <CreditCard style={{ fontSize: 18, color: "#FFA800" }} />;
      case "operativo":
          ...
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <p className="text-white-75 font-size-lg mb-0">
                Gestión y análisis de medios y esquemas de pago del Banco
                Central
              </p>
            </div>
          ...
          <div className="card-title">
            <h3 className="card-label">
              ...
              <span className="text-muted mt-2 font-weight-bold font-size-sm">
                Explora y analiza los datos del Apartado B
              </span>
            </h3>
          </div>
            ...
            <div className="d-flex align-items-center flex-grow-1 mb-4">
              <div className="input-icon input-icon-right w-100">
                <input
                  ...
                  placeholder="Buscar concepto, código, medio o esquema de pago..."
                  value={busqueda}
                ...
                <table className="table table-head-custom table-vertical-center overflow-hidden">
                  <thead>
                    <tr>
                      ...
                      <th>
                        <span className="text-dark-75">Medio de Pago</span>
                      </th>
                      <th>
                        <span className="text-dark-75">Esquema de Pago</span>
                      </th>
                      <th>
...
function getTipoConcepto(concepto) {
  ...
  if (conceptoLower.includes("pago")) return "pagos";
  if (conceptoLower.includes("esquema")) return "esquemas";
  if (conceptoLower.includes("medio")) return "medios";
  if (conceptoLower.includes("cantidad")) return "cantidad";
  if (conceptoLower.includes("monto")) return "monto";
  return "general";
}
...
function getCategoriaConcepto(concepto) {
  ...
  if (conceptoLower.includes("esquema")) return "esquema";
  if (conceptoLower.includes("medio")) return "medio";
  if (
