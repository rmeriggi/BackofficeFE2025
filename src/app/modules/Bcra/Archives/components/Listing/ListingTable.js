...
const filterData = (archivesData, filter) => {
    ...
    if (filter.file !== "" || filter.period !== "") {
        filteredData = archivesData.filter(account => {
            if (
                account.file.toLowerCase().includes(filter.file.toLowerCase()) ||
...
export function ListingTable( { archivesData } ) {
    ...
    const filteredData = filterData(archivesData, queryParams.filter);
    ...
    return (
        archivesData.length === 0 ? (
            <TableNoRecordsFoundMessage entities={"Archivos"}/>
        ) : (
            <PaginatedTable
                columns={columns}
                data={filteredData}
