...
export function CardDetail({history,match: {params: { id },}}) {
  ...
  const [message, setMessage] = useState("Los datos fueron modificados correctamente")
  ...
  const saveEditCard = async (values) => {
    try {
      ...
      setVariant('error')
      setMessage('Los datos no pudieron ser modificados correctamente. Por favor, volvé a intentar.')
      setOpenSnackbar(true)
    }
  }
