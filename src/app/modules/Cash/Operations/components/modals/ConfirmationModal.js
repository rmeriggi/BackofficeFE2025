...
import { authorize, reject } from '../../utils/service'
...
const ConfirmationModal = ({show, onHide, action, id}) => {
  ...
  const handleAction = async() => {
    try {
      if(action === "authorize") {
        await authorize(id)
        setOpenMessage("success", "La operación fue autorizada exitosamente.")
      } else {
        await reject(id)
        setOpenMessage("success", "La operación fue rechazada exitosamente.")
      }
      setTimeout(()=>{
