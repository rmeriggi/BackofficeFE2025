...
const Listing = ({setOpenMessage ,data }) => {
    ...
    const [action, setAction] = useState({status: "Activo", value: 4, idCredit: null})
    ...
    const handleClick = async() => {
        ...
        try {
            ...
            setOpenMessage("error", "Acción no realizada, por favor intente más tarde")
            setIsSubmitting(false)
        }
    }
    ...
    const newArr = credits.map(c => {
        ...
        const idEntity= products?.find(p => p.id == c.productName)?.idEntity || "Sin datos"
        const iva= products?.find(p => p.id == c.productName)?.iva || "Sin datos" 
        const status = creditsStatus?.find(s => s.id == c.status )?.status || "Sin datos"
        const entity = entitiesData.data?.find(e => e.id == idEntity)?.entity || "Sin Datos"
        const total = products?.find(p => p.id == c.productName)?.amount || "Sin datos"
        const name = clients?.find(x=> x.id == c.idClient)?.name || "Sin Datos"
        const surname = clients?.find(x=> x.id == c.idClient)?.lastName || "Sin Datos" 
        const dni = Number(clients?.find(x=> x.id == c.idClient)?.passport) || "Sin Datos"
        const email = clients?.find(x=> x.id == c.idClient)?.email || "Sin Datos"
        return {
            ...c,
            productName,
            status,
    ...
    return (
        <Card>
            ...
            >
                <div>
                    <span>¿Estas seguro que desea cambiar el estado a {action.status}?</span>
                </div>
            </ModalWrapper>
        </Card>
    )
