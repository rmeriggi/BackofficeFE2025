...
export function EditModal(props) {
    ...
    const catFilter = cat.filter((e)=>e.id == props.editInitialData.FX || e.id == 4 )
    ...
    const initialValues = {
        ...
        quantityFormat: parseFloat(props.editInitialData.quantity).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'),
        price: new Intl.NumberFormat("de-DE", {minimumFractionDigits:4,  maximumFractionDigits: 4}).format(props.editInitialData.price),
        amount: new Intl.NumberFormat("de-DE", {minimumFractionDigits:2,  maximumFractionDigits: 2}).format(props.editInitialData.amount),
    ...
    async function handleEdit(e) {     
      const values={
         ...
         notes:e.notes,
         FX:e.FX,
         tranferPrice:Number(e.transferPrice.replace(/\./g, '').replace(',', '.')),
        ...
        {({ handleSubmit, handleChange ,setFieldValue, values, isSubmitting }) => (
          <>
            <Modal.Body className="overlay overlay-block cursor-default">
                ...
                <div className="col-lg-4">
                    <label className="font-size-h7  col-form-label text-lg-left">TRANSFER PRICE</label> 
                    <Field
                         name="transferPrice"
                         component={Input}
                         type="text"
                    ...
                    <div className="col-lg-4">
                    <label className="font-size-h7 col-lg-4 col-form-label text-lg-left">RDO</label> 
                    <Field
                        name="performance"
                        component={Input}
                        placeholder=""
                        ...
                        label=""
                        readOnly
                        value={values.performance = props.editInitialData.operations === "COMPRA" ?
                          values.transferPrice.includes(',') ?
                          new Intl.NumberFormat("de-DE", {minimumFractionDigits:2,  maximumFractionDigits: 2}).format(((Number(values.transferPrice.replace(/\./g, '').replace(',', '.'))-Number(values.price.replace(/\./g, '').replace(',', '.')))/100)*(values.quantity)):
                          new Intl.NumberFormat("de-DE", {minimumFractionDigits:2,  maximumFractionDigits: 2}).format(((Number(values.transferPrice)-Number(values.price.replace(/\./g, '').replace(',', '.')))/100)*(values.quantity))
                        : values.transferPrice.includes(',') ?
                         new Intl.NumberFormat("de-DE", {minimumFractionDigits:2,  maximumFractionDigits: 2}).format(((Number(values.price.replace(/\./g, '').replace(',', '.'))-Number(values.transferPrice.replace(/\./g, '').replace(',', '.')))/100)*(values.quantity))  :
                         new Intl.NumberFormat("de-DE", {minimumFractionDigits:2,  maximumFractionDigits: 2}).format(((Number(values.price.replace(/\./g, '').replace(',', '.'))-Number(values.transferPrice))/100)*(values.quantity))}
                    />
                    </div>  
