...
export default function CardGraphic({children, title, data}) {
  ...
  return (
    <Grid item xs={12} md={3} className="bg-white mh-100 mt-5 rounded shadow-sm" style={{height: "auto", minWidth: "45%"}}>
      ...
      {title === "Operaciones" && <FooterBarGraphic transactions={data}/>}
      {title === "Volumen" && <FooterDoughnutGraphic volume={data}/>}
      {title === "Volumen Operado" && <FooterLineGraphic balances={data}/>}
      {title === "Comisiones" && <FooterLineGraphic balances={data}/>}
    </Grid>  
  )
}
