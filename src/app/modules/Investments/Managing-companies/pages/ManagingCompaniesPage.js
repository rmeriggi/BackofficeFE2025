...
import { managingCompaniesData } from "../mocks/managers";
...
const ManagingCompaniesPage = () => {
  ...
  const filteredCompanies = managingCompaniesData.filter((company) => {
    const categoryMatch =
      activeFilter === "all" ||
  ...
  const totalCompanies = managingCompaniesData.length;
  const totalAssets = managingCompaniesData.reduce(
    (sum, company) => sum + company.financials.totalAssets,
    0
  );
  const totalManagedFunds = managingCompaniesData.reduce(
    (sum, company) => sum + company.financials.managedFunds,
    0
  );
  /*   const activeCompanies = managingCompaniesData.filter(
    (company) => company.status === "Activa"
  ).length; */
  const totalInvestors = managingCompaniesData.reduce(
    (sum, company) => sum + company.totalInvestors,
    0
  );

  // Contar por categoría
  const categories = [
    ...new Set(managingCompaniesData.map((company) => company.category)),
  ];
  const categoryCounts = categories.map((category) => ({
    category,
    count: managingCompaniesData.filter(
          ...
          <div className="card-title">
            <h3 className="card-label">
              ...
              <span className="text-muted mt-2 font-weight-bold font-size-lg">
                Gestión integral de sociedades gerentes autorizadas
              </span>
            </h3>
          </div>
          ...
          <div className="d-flex justify-content-between flex-wrap mb-6">
            ...
            <div className="d-flex align-items-center">
              <div className="symbol symbol-50 symbol-light-info mr-4">
                <span className="symbol-label bg-white">
                  <Group style={{ fontSize: 24, color: "#8950FC" }} />
                </span>
              </div>
              <div>
                <div className="font-size-sm text-muted font-weight-bold">
                    ...
                    <div className="col-6">
                      <div className="d-flex flex-column">
                        ...
                        <span className="text-warning font-weight-bolder font-size-h5">
                          {company.totalInvestors.toLocaleString()}
                        </span>
                      </div>
                    </div>
                    <div className="col-6">
                      ...
                      <div className="mt-4 pt-4 border-top">
                        <div className="row mb-4">
                          ...
                          <div className="col-6">
                            ...
                            <span className="text-dark">
                              {company.management.portfolioManager}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="row mb-4">
                          <div className="col-6">
                            ...
                            <span className="text-dark font-weight-bold">
                              {company.financials.performanceFee}%
                            </span>
                          </div>
                          <div className="col-6">
                            <span className="text-muted font-weight-bold d-block mb-2">
                            ...
                            <span className="text-dark font-weight-bold">
                              {company.financials.managementFee}%
                            </span>
                          </div>
                        </div>
              ...
              <table className="table table-hover table-vertical-center">
                <thead>
                  <tr>
                    ...
                    <th className="min-w-100px">Inversores</th>
